...
var page1 = {
  ...
  init: function () {
    ...
    function resetMargin () {
      if(window.innerHeight < initHeight - 100 && contentFocus) {
        // 输入法弹出
    ...
    // 让高度不再变化
    pageContainer.style.height = initHeight + 'px';
    window.addEventListener('resize',resetMargin)
    // 点击下一页
    this.doms.next.onclick = function () {
      if(!page1.doms.txtAuthor.value.trim() || !page1.doms.txtContent.innerHTML.trim()) {
        alert('请输入祝福语！！！')
        return;
      }
      toPage(1);
  ...
  stopRecord: async function () {
    ...
    const resp = await utils.audioRecorder.stopAndUpload(
      `https://bless.yuanjin.tech/api/upload`,
      {
        ...
        accessKeyId: "LTAI5tJDTqYzsgsijSjupxUz",
        accessKeySecret: "3HHmj3DbHvwfM3fbOS8770Yiehnie9",
        bucket: "whil",
      }
    );
